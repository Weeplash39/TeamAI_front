...
import { ArrowLeft, Settings, Wifi, WifiOff, Trash2 } from "lucide-react";
...
function ProjectChatContent() {
  ...
  const { selectedAgent, selectedMode, clearMessages, getMessages } =
    useChatStore();
  const { llmConfig } = useSettingsStore();
  const { sendMessage, orchestrate, isConnected, isStreaming } = useWebSocket();
  ...
  const handleSendMessage = (message: string) => {
    if (selectedMode === "consultation") {
      ...
      orchestrate(message, selectedMode, llmConfig);
    }
  };
  ...
  const handleClearChat = () => {
    if (
      window.confirm(
        "Voulez-vous vraiment effacer tout l'historique de ce projet ?"
      )
    ) {
  ...
  const messageCount = getMessages(projectId!).length;
...
export function ProjectChat() {
  ...
  if (!projectId) {
    return <div>Erreur: Aucun projet sélectionné</div>;
  }
